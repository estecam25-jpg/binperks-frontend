...
function MerchantLoginContent() {
  ...
  useEffect(() => {
    // No client-side "already signed in?" probe here any more — the server
    // component in page.tsx redirects before this ever renders. Doing it here
    // as well raced the render and could only ever act after paint.
...
// Suspense boundary required because useSearchParams() opts the component into
// dynamic rendering.
//
// The fallback is NOT optional: with an empty <Suspense>, the prerendered HTML
// contains nothing and a merchant sees a blank screen until the client bundle
// hydrates. That was half of the blank-page bug.
