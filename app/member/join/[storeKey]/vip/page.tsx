...
export default function VipUpsellPage() {
  ...
  useEffect(() => {
    ...
    if (!m) { router.replace(`/member/join/${storeKey}/signup`); return }
  }, [router, storeKey])
  ...
  async function handleVip() {
    ...
    const res = await fetch('/api/join/vip', {
      ...
      body: JSON.stringify({
        ...
        successUrl: `${window.location.origin}/member/join/${storeKey}/thankyou?plan=vip`,
        cancelUrl:  `${window.location.origin}/member/join/${storeKey}/vip`,
      }),
    })
  ...
  function handleFree() {
    setLoading('free')
    router.push(`/member/join/${storeKey}/thankyou?plan=free`)
  }
